...
for f in sorted(listdir_nohidden(root_path)):
    ...
    # files_names = [f for f in sorted(listdir_nohidden(root_path))]
    # if files_names[-1].endswith("emotion.txt"):
    file_path = root_path/f # -2 because emotions are there
    file = open(str(root_path/f), 'r')
    ...
    # if you use Windows comment next line
    cropped_labeled_path = PurePosixPath(str(cropped_path/f))
